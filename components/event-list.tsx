  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {events.map((event, index) => (
        ...
        >
          <Card
            className="overflow-hidden glass border-white/20 transition-all hover:shadow-2xl hover:shadow-blue-500/10 group h-full flex flex-col rounded-3xl"
          >
            <CardHeader className="p-0 relative h-52 overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent z-10" />
              <div className="absolute top-4 left-4 z-20">
                <Badge className="glass border-white/30 text-white bg-white/10 backdrop-blur-md px-3 py-1">
                  Upcoming
                </Badge>
              </div>
              <div className="h-full w-full bg-blue-100 flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                <svg
            ...
            <CardFooter className="p-6 pt-0 mt-auto">
              <Link href={`/events/${event.id}`} className="w-full">
                <Button className="w-full h-12 rounded-2xl bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/10 group-hover:scale-[1.02] transition-all">
                  Get My Ticket
